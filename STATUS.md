...
**Cập nhật lần cuối:** 2026-05-14 (Stage 7 rewrite — KHÔNG upload, grade table auto + multi-intent recommend/regulation/prereq)
...
- **Stage 7 (2026-05-14, scope chuẩn sau 3 vòng user clarify)**: chatbot **không upload file** — user chỉ chọn ngành + HK target ở sidebar → grade table tự sinh từ curriculum → user gõ điểm vào bảng → chat 3 intent (recommend / regulation / prereq).
- Đã viết 9 thay đổi: `grade_table_builder.py` (mới), `profile_loader.build_profile_from_grades` (sửa), `intent_classifier.py` (mới, heuristic 8/8 pass), `regulation_qa.py` (mới, inject regulations.json), `prereq_qa.py` (mới, extract môn + graph lookup), `rag_pipeline.chat()` dispatch (sửa), `api/main.py` 2 endpoint mới `GET /curriculum/grade-table` + `POST /chat/v2` (sửa), `frontend/app.py` rewrite `st.data_editor` grade table (rewrite). Backend Stage 1-5 (retriever/constraint/Qwen LoRA) **giữ nguyên 100%**.
- E2E test stub mode 3 intent với SV CS HK3 (18 môn đã nhập điểm):
  - **A. Recommend** "AI/ML chọn môn HK3?" → Python, R, Matlab, ... pool 7 môn tu_chon HK3.
  - **B. Regulation** "TC tối đa/HK?" → trả full regulations IUH (TC min 12 / max 30 + cảnh báo + tốt nghiệp).
  - **C. Prereq** "Học Máy học 015028 ở HK5?" → detect mã, lookup graph: cần Trí tuệ nhân tạo (001954), user CHƯA → "Chưa đủ điều kiện".
- LLM mode (Qwen) chưa test lại — chỉ thêm dispatch layer trên cùng, core inference không đổi. User có thể bật `$env:USE_LLM=1` để demo full natural language.
...
- 2026-05-14: User refine scope thêm: chatbot phải gợi ý đúng **môn tự chọn của HK đang đăng ký** (không phải bắt buộc + không phải HK khác). Thêm `hk_target` xuyên suốt pipeline: `ProfilePayload.hk_target`, `RagPipeline.chat(hk_target, only_tu_chon)`, `build_chat_user_message(hk_target=...)` mention rõ trong prompt. Pipeline lọc 2 lớp: (a) loại bỏ `loai != "tu_chon"` + `hk_chuan != hk_target`; (b) **force-include toàn bộ tu_chon HK target của ngành user** (đảm bảo Qwen thấy hết pool 9-15 môn, không phụ thuộc top-K retriever). Thêm `load_required_courses(nganh, hk_max)` đọc curriculum_graph để auto-add môn bắt buộc HK < target vào completed (vì xlsx khoa cung cấp chỉ có môn tu_chon, mặc nhiên SV đã pass bắt buộc theo curriculum).
- 2026-05-14: Sửa `_read_csv_smart` auto-detect separator `|` (CS/DS/SE/IT) vs `,` (IS) bằng đếm trong header line — pandas sniffer sep=None nhầm khi cột có Unicode tiếng Việt. Frontend thêm input số `Học kỳ chuẩn bị đăng ký` (default = profile.current_hk) + auto-merge `required_courses` vào completed payload.
- 2026-05-14: Thêm field `context_docs` vào `ChatResponseModel` — UI hiển thị "Pool môn bot đã xét" (sau filter loại+HK+constraint) để user thấy bot không hallucinate. Test E2E với SV thực CS `IDSinhVien=1610001`, HK target=3, định hướng AI/ML, USE_LLM=1: pool 7 môn tu_chon HK3 (Python, R, Matlab, ƯD hóa học, ...), Qwen chọn 2 môn (R + Công nghệ chuyển đổi số) với giải thích 3 phần (lý do + kết nối điểm cao "Lập trình phân tích dữ liệu 1 (8.4)" + hỗ trợ định hướng AI/ML). Timing 45s (Qwen sinh 768 token verbose).
- 2026-05-14: User clarify lần 3 (scope cuối): chatbot **không upload file**, chỉ chọn ngành + HK ở sidebar → grade table tự sinh từ curriculum → user nhập điểm bằng `st.data_editor`. Bot còn phải trả lời thêm 2 loại câu hỏi: quy định học vụ + khả năng đăng ký 1 môn cụ thể (prereq). Save memory feedback `chatbot-ux-scope` cho future sessions.
- 2026-05-14: **Stage 7 rewrite multi-intent** — 9 files (4 mới + 5 sửa):
  - `src/data/grade_table_builder.py` (mới): `build_grade_table(nganh, hk_target)` đọc curriculum_graph JSON → list môn HK1..HK(target-1) sort theo (hk_chuan, bat_buoc-first, ma_mon). Test CS HK3 → 19 môn.
  - `src/data/profile_loader.py` (sửa): thêm `build_profile_from_grades(nganh, grades, curriculum_rows, hk_target)` build `StudentProfile` từ dict điểm + curriculum metadata (không cần xlsx).
  - `src/generation/intent_classifier.py` (mới): `classify_intent(query) → recommend / regulation / prereq` heuristic keyword + regex pattern. Smoke 8/8 pass.
  - `src/generation/regulation_qa.py` (mới): `RegulationQA` load `regulations.json` → format markdown 9 nhóm rule → inject toàn bộ vào Qwen prompt. Stub fallback trả raw markdown.
  - `src/retrieval/prereq_qa.py` (mới): `PrereqQA` extract mã môn 6 chữ số + fuzzy match tên môn (NFD normalize) → query `ConstraintChecker.get_prereqs` → format info block (đủ/thiếu prereq) → Qwen format. Stub fallback trả markdown.
  - `src/rag_pipeline.py` (sửa): `chat()` đầu dispatch `classify_intent` → 3 nhánh `_chat_recommend / _chat_regulation / _chat_prereq`. `ChatResult` thêm `intent` + `target_courses`.
  - `api/main.py` (sửa): 2 endpoint mới — `GET /curriculum/grade-table?nganh=&hk_target=` và `POST /chat/v2` (request `nganh + hk_target + grades + messages`). Response thêm `intent` + `target_courses`.
  - `frontend/app.py` (rewrite): bỏ `st.file_uploader`, thay `st.data_editor` bảng grade. Sidebar chọn ngành + HK → bấm "Tạo bảng điểm" → bảng render. User gõ điểm → chat input → POST `/chat/v2`. Hiển thị intent label + bảng recommendations (intent A) hoặc target courses (intent C).
- 2026-05-14: E2E test stub mode 3 intent với SV CS HK3 (18 môn đã nhập điểm):
  - **A. Recommend** "AI/ML chọn môn HK3?" → 5 môn tu_chon HK3 (Python, R, Matlab, ...).
  - **B. Regulation** "TC tối đa/HK?" → trả full quy định IUH (TC min 12, max 30, cảnh báo học tập, tốt nghiệp GPA ≥ 2.0, bảng quy đổi điểm, ...).
  - **C. Prereq** "Học Máy học 015028 ở HK5?" → extract `CS_015028`, graph lookup: cần Trí tuệ nhân tạo (001954) (loại a học trước), user CHƯA → kết luận "Chưa đủ điều kiện".
- 2026-05-14: **Pipeline giờ ĐÚNG scope user clarify (vòng 3)**. Backend Stage 1-5 retriever/constraint/Qwen LoRA + augmentation/embedding/eval (Stage 2-4) giữ nguyên 100%, chỉ thêm 3 file Q&A handler + 1 intent router + grade table service.
- 2026-05-15: User báo bot không giải thích. Debug: lần đầu mình đổi prompt sang few-shot dài + max_new_tokens=1024 → prompt input tăng từ ~800 lên ~1500 tokens → Qwen LoRA sinh chậm 7x (300s+) → response truncate giữa chừng → kết quả 2 môn ngắn không giải thích. Thử `disable_adapter()` để dùng base Qwen (không LoRA) cũng timeout 400s+ trên 4-bit (PEFT context-manager overhead trên bitsandbytes layer). 3 fix apply:
  1. **Revert prompt** về format đơn giản 14/5 (instruction 3 mục: gợi ý 3-5 môn / giải thích / cảnh báo).
  2. **Giữ LoRA** (bỏ `disable_lora` flag) — Qwen LoRA cũng giải thích được nếu profile có hook hoặc prompt rõ.
  3. **Thêm `enrich_short_response`** ở `prompt_templates.py` — nếu Qwen trả < 300 chars hoặc < 3 môn, Python sinh fallback explanation block dựa: detect cluster từ keyword query (AI/ML, Web, Data, ...), trích top điểm cao từ profile, template hoá lý do cho top 5 môn trong context_docs. Cũng cảnh báo user nhập thêm điểm môn nền tảng để Qwen có hook tốt hơn.
  4. **Giảm `max_new_tokens` default** 1024 → 768 — đủ cho 3-5 môn × giải thích 3 dòng, latency ~42-60s/turn.
- 2026-05-15: Test E2E LLM mode lại với chính grades user paste (CS HK3, 8 môn HK1+HK2 bắt buộc, query "Tôi muốn theo làm AI"): wall 42.9s, Qwen tự sinh 3 môn (Công nghệ chuyển đổi số, Tính toán số & Matlab, Ngôn ngữ R) với block `### Giải thích` 3 phần (Lý do + Nhận xét) — **giải thích đầy đủ**, không cần kick fallback. **Stage 7 explanation issue đã fix**.
- 2026-05-15: User hỏi "model có dựa vào điểm môn bắt buộc để tư vấn không?" — phát hiện hạn chế: profile_summary chỉ inject top 5 điểm CAO + GPA, không phân biệt cluster, Qwen khó phân tích "em mạnh nhóm X, yếu nhóm Y". Thêm `COURSE_CLUSTERS` mapping (7 cluster: Lập trình nền tảng, Toán & Lý thuyết, AI/ML, Đồ họa & CV, DB & Dữ liệu, Web/Java/.NET, Đại cương) trong `src/data/profile_loader.py` + function `classify_course(ten_mon) → cluster` (substring match không dấu). `build_profile_from_grades` compute `cluster_avg = {cluster: {avg, n, courses}}`. `summary_markdown()` append block "Điểm trung bình theo nhóm môn" (sort desc, bỏ "Khác"). Test E2E LLM: Qwen reference rõ ràng "Môi trường IT khá tốt (8.00)", "Lập trình và Toán (7.00)" trong reasoning — **model giờ dựa vào điểm môn bắt buộc theo cluster**. Wall 39.6s.
